...
BUILD_DIR="${BUILD_DIR:-${REPO_ROOT}/build/ca-game-of-life}"
GENERATOR="${GENERATOR:-Ninja}"
TARGET="genesys_terminal_application"
EXAMPLE="smarts/Smart_CellularAutomataGameOfLife.cpp"
EXECUTABLE="${BUILD_DIR}/source/applications/terminal/genesys_terminal_application"

log() {
    printf '[ca-game-of-life] %s\n' "$*"
}

if ! command -v "${CMAKE_BIN}" >/dev/null 2>&1; then
    printf '[ca-game-of-life] error: cmake not found. Set CMAKE_BIN or install CMake.\n' >&2
    exit 1
fi
...
log "Running Game of Life CellularAutomataComp example"
"${EXECUTABLE}"

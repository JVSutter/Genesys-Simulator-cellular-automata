...
BUILD_DIR="${BUILD_DIR:-${REPO_ROOT}/build/ca-rule90}"
GENERATOR="${GENERATOR:-Ninja}"
TARGET="genesys_terminal_application"
EXAMPLE="smarts/Smart_CellularAutomataRule90.cpp"
...
if ! command -v "${CMAKE_BIN}" >/dev/null 2>&1; then
    printf '[ca-rule90] error: cmake not found. Set CMAKE_BIN or install CMake.\n' >&2
    exit 1
fi

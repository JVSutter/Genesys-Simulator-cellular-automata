...
BUILD_DIR="${BUILD_DIR:-${REPO_ROOT}/build/ca-nd-lattice}"
GENERATOR="${GENERATOR:-Ninja}"
TARGET="genesys_terminal_application"
EXAMPLE="smarts/Smart_CellularAutomataNDLattice.cpp"
EXECUTABLE="${BUILD_DIR}/source/applications/terminal/genesys_terminal_application"

log() {
    printf '[ca-nd-lattice] %s\n' "$*"
}

if ! command -v "${CMAKE_BIN}" >/dev/null 2>&1; then
    printf '[ca-nd-lattice] error: cmake not found. Set CMAKE_BIN or install CMake.\n' >&2
    exit 1
fi
...
log "Running N-dimensional CellularAutomataComp example"
"${EXECUTABLE}"

...
BUILD_DIR="${BUILD_DIR:-${REPO_ROOT}/build/ca-update-policies}"
GENERATOR="${GENERATOR:-Ninja}"
TARGET="genesys_terminal_application"
EXAMPLE="smarts/Smart_CellularAutomataUpdatePolicies.cpp"
EXECUTABLE="${BUILD_DIR}/source/applications/terminal/genesys_terminal_application"

log() {
    printf '[ca-update-policies] %s\n' "$*"
}

if ! command -v "${CMAKE_BIN}" >/dev/null 2>&1; then
    printf '[ca-update-policies] error: cmake not found. Set CMAKE_BIN or install CMake.\n' >&2
    exit 1
fi
...
log "Running CellularAutomataComp update policy example"
"${EXECUTABLE}"
